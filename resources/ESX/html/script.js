...
var ShowroomFunctions = {
    ...
    GetCars: function () {
        $.post('https://ESX/np-ui:showroomChangeCar', JSON.stringify({ model: cars[selectedId].model }), function (cb) {
            ShowroomFunctions.SelectWaitEnd()
		console.log("Showroom: Loading2...");
            left = 64
...
$(document).ready(function () {
    ...
    window.addEventListener('message', (event) => {
        ...
        if (data.meta.message = "done") {
            ...
            $.post('https://ESX/np-ui:application-closed', JSON.stringify({ name: "showroom" }));
        }
    })
});
...
$(document).on("click", "#purchase", function () {
    canPress = true
    $.post('https://ESX/showroomPurchaseCurrentVehicle', JSON.stringify({ model: cars[selectedId].model, price:cars[selectedId].price, zoneName: zone, label:cars[selectedId].label, category:cars[selectedId].category}), function(cb){
     if( cb.meta.ok) {
        ShowroomFunctions.Close()
        canPress = false
     } else {
