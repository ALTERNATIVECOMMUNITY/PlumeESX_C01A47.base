...
AddEventHandler("np-ui:voice:setvoiceRange", function(proximity)
    sendAppEvent("hud", {
...
AddEventHandler("np-ui:voice:setRadioActive", function()
    sendAppEvent("hud", {
        voiceActive = false,
...
AddEventHandler("np-ui:voice:setRadioActiveFinished", function()
    sendAppEvent("hud", {
        voiceActive = false,
...
RegisterNetEvent("np-ui:voice:updateHud")
AddEventHandler("np-ui:voice:updateHud", function(level)
    voiceLevel = level
...
--food and water updates
local startedFoodWaterUpdates = false
function startFoodWaterUpdates()
    makeHudDirty()
    if startedFoodWaterUpdates then return end
    local prevHealth = -1
    local prevArmor = -1
    Citizen.CreateThread(function()
        startedFoodWaterUpdates = true
        while true do
            --print('running food and water')
            local hunger 
            local thirst 
            local stress
            ...
            setHudValue("food", hunger)
            setHudValue("water", thirst)
            setHudValue("stress", stress)
            Citizen.Wait(1500)
        end
    end)
end
