		...
		for _,i in pairs(lit) do
			...
			if DoesEntityExist(closestObject) then
				...
				if #(pedCoords - litCoords) <= 5.0 then
					--if #(pedCoords - sitCoords) <= 2.0 and not IsEntityPlayingAnim(PlayerPedId(), 'anim@heists@box_carry@', 'idle', 3) then
						--hintToDisplay(Config.Language.do_action)
						--if IsControlJustPressed(0, Config.Press.do_action) then
							WarMenu.OpenMenu('hopital')
						--end
					if IsEntityAttachedToEntity(closestObject, PlayerPedId()) == false and not IsEntityPlayingAnim(PlayerPedId(), 'anim@heists@box_carry@', 'idle', 3) then
						if #(pedCoords - pickupCoords) <= 2.0 then
						...
						if #(pedCoords - pickupCoords2) <= 1.5 and prop_amb == true then
							CancelEvent()
						else
							hintToDisplay(Config.Language.take_bed)
							if IsControlJustPressed(0, Config.Press.take_bed) then
...
RegisterNetEvent('stretcher:findPlayer')
AddEventHandler('stretcher:findPlayer', function()
	local player = PlayerPedId()
	local coords = GetEntityCoords(player)
	local cplayer, dis = ESX.Game.GetClosestPlayer(coords)
	if cplayer and dis < 3 then 
		print('sent')
		TriggerServerEvent('stretcher:sendTo', GetPlayerServerId(cplayer))
	end
end)

RegisterNetEvent('stretcher:putOn')
AddEventHandler('stretcher:putOn', function()
	local player = PlayerPedId()
	local coords = GetEntityCoords(player)
	local closestObject = GetClosestVehicle(coords, 3.0, GetHashKey("stretcher"), 70)
	if DoesEntityExist(closestObject) then
		print('alive')
