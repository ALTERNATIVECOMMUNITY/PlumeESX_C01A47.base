...
RegisterNetEvent('pe-fake-plate:Peak')
AddEventHandler('pe-fake-plate:Peak', function()
    local veh,dis = ESX.Game.GetClosestVehicle(GetEntityCoords(PlayerPedId()))
    local plate = GetVehicleNumberPlateText(veh)
    local hasFake = false
    for _, p in pairs(fakePlates) do
        print(p)
        if p == plate then
            hasFake = true
        end
    ...
    if hasFake then
        TriggerServerEvent('pe-fake-plate:startReturnPlate')
    else
        TriggerServerEvent('pe-fake-plate:startFakePlate')
    end
end)

RegisterNetEvent('pe-fake-plate:updateFakes')
AddEventHandler('pe-fake-plate:updateFakes', function(fake)
    table.insert(fakePlates, fake)
    exports["onyxLocksystem"]:givePlayerKeys(fake)
end)
