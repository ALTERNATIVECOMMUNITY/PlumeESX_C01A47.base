...
RegisterNetEvent('pe-fake-plate:startFakePlate')
AddEventHandler('pe-fake-plate:startFakePlate', function()
    ...
    if xPlayer.getInventoryItem('fakeplate').count >= 1 then
        if Config.ownerRestricted then
            if identifier then
                fetchScalar('SELECT 1 FROM owned_vehicles WHERE (owner, plate) = (@owner, @plate)', {
                    ['owner']          = identifier,
                    ['plate']          = plate,
                }, function(results)
                    if results then
        ...
        TriggerClientEvent('pe-fake-plate:notifyError', source, 'You dont have a fake plate...')
    end
end)

RegisterNetEvent('pe-fake-plate:startReturnPlate')
AddEventHandler('pe-fake-plate:startReturnPlate', function()
    local source        = source
    local ped           = GetPlayerPed(source)
    local netVehicle    = GetVehiclePedIsIn(ped, true)
    ...
    if originalPlate ~= nil and fakePlate ~= nil then
        if cl_OriginalPlate == originalPlate and cl_FakePlate == fakePlate then
            if plateType == 'fake' then
                ...
                applyingPlate   = false
                TriggerClientEvent('pe-fake-plate:updateFakes', source, fakePlate)
