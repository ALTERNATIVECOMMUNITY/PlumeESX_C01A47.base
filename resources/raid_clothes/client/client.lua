...
local clothesSet = false
local clothesMenu = false
local player
local changeOfClothes = {}
local policeCloset = false
...
RegisterCommand("h0", function(source, args, raw)
       TriggerEvent('raid_clothes:toggleprop', 'hats', false)
       RequestAnimDict("missheistdockssetup1hardhat@")
       while not HasAnimDictLoaded("missheistdockssetup1hardhat@") do
            Citizen.Wait(100)
       end
       TaskPlayAnim(PlayerPedId(), "missheistdockssetup1hardhat@", "put_on_hat", 8.0, 8.0, -1, 0, 0.3, true, true, true)
end, false)
RegisterCommand("g0", function(source, args, raw)
       TriggerEvent('raid_clothes:toggleprop', 'glasses', false)
       RequestAnimDict("clothingspecs")
       while not HasAnimDictLoaded("clothingspecs") do
            Citizen.Wait(100)
	   end
       TaskPlayAnim(PlayerPedId(), "clothingspecs", "try_glasses_neutral_c", 8.0, 8.0, -1, 0, 0.3, true, true, true)
end, false)
RegisterCommand("m0", function(source, args, raw)
       TriggerEvent('raid_clothes:toggleprop', 'masks', false)
       RequestAnimDict("mp_masks@on_foot")
       while not HasAnimDictLoaded("mp_masks@on_foot") do
            Citizen.Wait(100)
	   end
       TaskPlayAnim(PlayerPedId(), "mp_masks@on_foot", "put_on_mask", 8.0, 8.0, -1, 0, 0.3, true, true, true)
end, false)

RegisterNetEvent("raid_clothes:policeRemoveProps")
AddEventHandler("raid_clothes:policeRemoveProps", function()
    TriggerEvent('raid_clothes:toggleprop', 'masks', true)
    TriggerEvent('raid_clothes:toggleprop', 'glasses', true)
    TriggerEvent('raid_clothes:toggleprop', 'hats', true)
end)

RegisterNetEvent('raid_clothes:policeRemovePropsRadial')
AddEventHandler('raid_clothes:policeRemovePropsRadial', function()
	local closestPlayer, closestDistance = GetClosestPlayer()
	if closestDistance ~= -1 and closestDistance<1 then
        print(GetPlayerServerId(closestPlayer))
		TriggerServerEvent('raid_clothes:sendRemove', GetPlayerServerId(closestPlayer))
	end
end)
...
function ToggleProps(data, police)
    ...
    if (selectedValue > -1 and not police) then
...
function SetSkin(model, setDefault)
    ...
    if IsModelInCdimage(model) and IsModelValid(model) then
        ...
        if setDefault and model ~= nil then--and not isCustomSkin(model) 
...
function EnableGUI(enable, menu)
    ...
    SetPlayerLockon(PlayerId(), enable)
    if (not enable) then
        SaveToggleProps()
        oldPed = {}
    end
...
function Save(save)
    ...
    SetDisableAmbientMeleeMove(PlayerPedId(), false)
	...
	for i = 1, #shops do
        ...
        if comparedst < 20.0 then
            if comparedst < 5.0 then
                ...
                exports["np-ui"]:showInteraction('[E] to open Clothing Shop')
                interaction = true
                elseif shops == tattoosShops then
                exports["np-ui"]:showInteraction('[E] to open Tattoo Shop')
                interaction = true
                elseif shops == barberShops then
                 exports["np-ui"]:showInteraction('[E] to open Barber Shop')
                 interaction = true
                end
            else
                interaction = false
...
Citizen.CreateThread(function()
	while true do
        ...
        elseif startingMenu or policeCloset then
            menu = "clothesmenu"
        end
...
AddEventHandler("raid_clothes:setclothes", function(gen, data,alreadyExist)
...
RegisterNetEvent("raid_clothes:registered")
AddEventHandler("raid_clothes:registered", function()
    registered = true
end)
