    ...
    while true do
		...
		if LeaveRunning then
			...
			local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
				...
				if IsPedInAnyVehicle(playerPed, false) and IsControlPressed(2, 75) and not IsEntityDead(playerPed) and isDriving(playerPed, vehicle) then
                	Citizen.Wait(150)
				...
				if IsPedInAnyVehicle(PlayerPedId(), false) and isDriving(playerPed, vehicle) then
...
-----------------------------------------------------------------------------
RegisterNetEvent('vehcontrol:openExternalRadial')
AddEventHandler('vehcontrol:openExternalRadial', function()
	openExternal()
end)
...
function isDriving(ped, veh)
	
	if tostring(GetPedInVehicleSeat(veh, -1)) == tostring(ped) then
		return true 
	end
	return false
end
