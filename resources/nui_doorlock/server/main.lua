...
AddEventHandler('nui_doorlock:updateState', function(doorID, locked, src, usedLockpick, usedThermite)
	...
	if not IsAuthorized(xPlayer, Config.DoorList[doorID], doorInfo[doorID], usedLockpick, usedThermite) then
		return
	end
...
function IsAuthorized(xPlayer, doorID, locked, usedLockpick,usedThermite)
	local jobName, grade = {}, {}
	...
	if doorID.thermite and usedThermite then
		count = xPlayer.getInventoryItem('thermite').count
		if count and count >= 1 then canOpen = true 
	...
	if not canOpen and doorID.items then
		...
		for k,v in pairs(doorID.items) do
			...
			if count and count >= 1 then
				canOpen = true
				local consumables = { ['ticket']=1, ['Green_Keycard']=1 }
				if locked and consumables[v] then
					xPlayer.removeInventoryItem(v, 1)
				end
				if v == 'Green_Keycard' then
					xPlayer.triggerEvent('efe:ucuncukapidoorlock')
				end
				break
			end
		end
