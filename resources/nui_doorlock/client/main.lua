...
RegisterNetEvent('esx_thermite:onUse') -- Modify for whichever lockpick you're using
AddEventHandler('esx_thermite:onUse', function()
	if not isDead and not isCuffed and closestDoor and closestV.thermite then
		TaskTurnPedToFaceCoord(playerPed, closestV.textCoords, 0)
		Citizen.Wait(300)
		local count = 0
		while GetIsTaskActive(playerPed, 225) do Citizen.Wait(10) count = count + 1 if count == 150 then break end end
		...
		print(closestDoor)
		print(locked)
		if closestV.name == 'bobcat1' then
			TriggerEvent('efe:firstdoor', GetPlayerServerId(PlayerId()), closestDoor)
		elseif closestV.name == 'bobcat2' then
			TriggerEvent('efe:secondoor', GetPlayerServerId(PlayerId()), closestDoor)
		end -- Broadcast new state of the door to everyone
	end
end)
