...
AddEventHandler('EasyAdmin:TeleportRequest', function(id, tgtCoordsx, tgtCoordsy, tgtCoordsz)
	if id then
		if (tgtCoordsx == 0.0 and tgtCoordsy == 0.0 and tgtCoordsz == 0.0) then
			local tgtPed = GetPlayerPed(GetPlayerFromServerId(id))
			tgtCoords = GetEntityCoords(tgtPed)
		end
		SetEntityCoords(PlayerPedId(), tgtCoordsx, tgtCoordsy, tgtCoordsz,0,0,0, false)
	else
		SetEntityCoords(PlayerPedId(), tgtCoordsx, tgtCoordsy, tgtCoordsz,0,0,0, false)
	end
end)
