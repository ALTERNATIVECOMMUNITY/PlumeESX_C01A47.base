...
local IdentifierTables = {
    ...
    {table = "banking_access", column = "owner_identifier"},
    {table = "transaction_history", column = "identifier"},
    {table = "inventories", column = "identifier"}, 
    {table = "phone_contacts", column = "identifier"},
    {table = "disc_ammo", column = "owner"},
    {table = "outfits", column = "identifier"},
    {table = "user_licenses", column = "owner"},
...
AddEventHandler('kashactersS:CharacterChosen', function(charid, ischar)
    ...
    if type(charid) == "number" and string.len(charid) == 1 and type(ischar) == "boolean" then
        ...
        if ischar == true then
            new = false
            spawn = { x = -268.5494, y =  -956.4264, z = 31.21753 }--GetSpawnPos(src)
        else
            
            TriggerClientEvent('skinchanger:loadDefaultModel', src, true, cb)
            --TriggerClientEvent('raid_clothes:openmenu', src, true, cb)
            spawn = { x = -268.5494, y =  -956.4264, z = 31.21753 } -- DEFAULT SPAWN POSITION -271.0286, -957.4813, 31.21753
            ---268.5494, -956.4264, 31.21753
        end
