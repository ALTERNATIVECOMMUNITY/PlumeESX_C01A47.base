...
RegisterCommand('hire', function(source, args, raw)
	...
	if checkPriv(job, PlayerData.job.grade) then
		TriggerServerEvent("esx_joblisting:setJobPlayer", job, args[1])
	end
end)

RegisterCommand('promote', function(source, args, raw)
	local job = PlayerData.job.name
	if args[1] ~= nil and args[2] ~= nil and checkPriv(job, PlayerData.job.grade) then
		TriggerServerEvent("esx_joblisting:promotePlayer", job, args[1], args[2])
	end
end)
...
RegisterCommand('fire', function(source, args, raw)
	local job = PlayerData.job.name
	if checkPriv(job, PlayerData.job.grade) then
		TriggerServerEvent("esx_joblisting:setJobPlayer", job, args[1], 0, true)
	end
end)

function checkPriv(job,grade)
	local boss 
	local jobs = {mosleys_mech = 2, mount_zonah = 1, police = 4, towing = 2}
	if jobs[job] ~= nil then
		if jobs[job] == grade then
			return true
		end
	end
	return false
