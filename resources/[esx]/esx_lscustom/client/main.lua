...
AddEventHandler('bt-polyzone:enter', function(name)
    if name == "bennysOlympic" then
        atShop = true
    elseif name == "bennysSandy" then
        atShop = true
	end
end)
...
RegisterNetEvent('esx_lscustom:bennysRadial')
AddEventHandler('esx_lscustom:bennysRadial', function()
	repairBennys()
end)
--Bennys addon
function repairBennys()
	local ped, veh, dmg
	ped = PlayerPedId()
	veh = GetVehiclePedIsIn(ped)
	dmg = GetVehicleBodyHealth(veh)
	if veh ~= -1 and dmg < 990 then
		apprasial = math.floor((1000 - dmg))
		exports["br-menu"]:SetTitle("Bennys")
		exports["br-menu"]:SubMenu('Repair Shop' , 'Body Repair' , 'menuone' )
		exports["br-menu"]:AddButton('Repair Price' , 'Price: '..apprasial ,'esx_lscustom:SendBill' ,true , 'price')
	end
end

RegisterNetEvent('esx_lscustom:SendBill')
AddEventHandler('esx_lscustom:SendBill', function()
	TriggerServerEvent('esx_lscustom:buyRepair', tonumber(apprasial))
end)
