...
local Keys = {
  ...
  local pilot
  local interaction1 = false
  local interaction2 = false
  local interactionFork1 = false
  local interactionFork2 = false
  ...
  function ZrespPaczuszke(a)
      ...
      elseif a=='lody'then 
          paczuszka=CreateObject(GetHashKey('prop_boxpile_06a'),kartonKoord2.x,kartonKoord2.y,kartonKoord2.z-0.95,true,true,true)
          SetEntityAsMissionEntity(paczuszka)
          SetEntityDynamic(paczuszka,true)
          FreezeEntityPosition(paczuszka,false)
      ...
      elseif a=='napoje'then 
          paczuszka=CreateObject(GetHashKey('prop_boxpile_06a'),kartonKoord4.x,kartonKoord4.y,kartonKoord4.z-0.95,true,true,true)
          SetEntityAsMissionEntity(paczuszka)
          SetEntityDynamic(paczuszka,true)
          FreezeEntityPosition(paczuszka,false)
  ...
  AddEventHandler('tostdostawa:rampanpcdawaj',function(a)
      ...
      if a=='1'then
          ...
          TaskVehiclePark(pilot,transport,rampaCoord.x,rampaCoord.y,rampaCoord.z,266.0,1,30.0,false)
      ...
      elseif a=='2'then
          ...
          SetBlockingOfNonTemporaryEvents(pilot,true)
          SetEntityInvincible(pilot,true)
          TaskVehiclePark(pilot,transport,rampaCoord2.x,rampaCoord2.y,rampaCoord2.z,89.59,1,30.0,false)
          SetDriveTaskDrivingStyle(pilot,263100)
          SetPedKeepTask(pilot,true)
          TriggerEvent(Notify,'The driver is on his way.')
          zrespione=true
...
Citizen.CreateThread(function()
    while true do
        ...
        if distCar <= 2.0 then
            ...
            if not interactionFork1 then
                exports["np-ui"]:showInteraction('[E] Take forklift')
                interactionFork1 = true
            end
            if IsControlJustPressed(0, Keys['E']) then 
                interactionFork1 = false
                PozyczPojazd('1')
            end
        else 
            interactionFork1 = false
        ...
        if zlecDist <= 1.0 then
            ...
            --exports["np-ui"]:showInteraction('[E] Take order; [G] Start')
            if PrzejetyHangar == 1 then 
                exports["np-ui"]:showInteraction('[E] Take order')
                interaction1 = true
            elseif not interaction1 then
                exports["np-ui"]:showInteraction('[G] Take over hanger')
                interaction1 = true
            end
            ...
            elseif IsControlJustPressed(0, Keys['G']) then
                ...
                if PrzejetyHangar == 1 then
                    ...
                    TriggerEvent(Notify,'You have signed out of the Hangar')
                else
        ...
        --if distCar > 1.0 and zlecDist > 1.0 then 
            --exports["np-ui"]:hideInteraction()
       -- end
    end
end)
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(3)
            ...
            --if not interaction and distCar2 >= 1.0 then 
               -- exports["np-ui"]:hideInteraction()
                --interaction = false
            --end
        end
  ...
  function PozyczPojazd(a)
      if a=='1'then
          if PojazdWyjety==0 then
              ...
              exports["onyxLocksystem"]:givePlayerKeys(GetVehicleNumberPlateText(wozek))
          else
              PojazdWyjety=0
