...
local HasAlreadyEnteredMarker, isDead, isHandcuffed, hasAlreadyJoined, playerInService, atBennys = false, false, false, false, false, false
...
Citizen.CreateThread(function()
	...
	TriggerServerEvent('esx_service:activateService', 'police', 10)
	ESX.PlayerData = ESX.GetPlayerData()
end)

RegisterNetEvent("esx_policejob:duty")
AddEventHandler("esx_policejob:duty", function(msg)
	local awaitService
	ESX.TriggerServerCallback('esx_service:isInService', function(isInService)
		if not isInService then
			...
			if Config.MaxInService == -1 then
				ESX.TriggerServerCallback('esx_service:enableService', function(canTakeService, maxInService, inServiceCount)
					if not canTakeService then
						...
						exports['mythic_notify']:DoHudText('inform', 'You went 10-41')
					end
				end, 'police')
			else 
				ESX.TriggerServerCallback('esx_service:enableService', function(canTakeService, maxInService, inServiceCount)
			...
			exports['mythic_notify']:DoHudText('inform', 'You went 10-42')
			awaitService = true
		end
	end, 'police')
...
Citizen.CreateThread(function()
	...
	exports["bt-target"]:AddBoxZone("PoliceArmory", vector3(482.4, -995.0, 30.69), 0.4, 0.6, {
		name="PoliceArmory",
		heading=3.0,
		debugPoly=false,
		minZ=30.4,
		maxZ=30.75
			...
			options = {
				{
					event = "esx_policejob:openArmory",
...
RegisterNetEvent('esx_policejob:openArmory')
AddEventHandler('esx_policejob:openArmory', function()
	exports["mf-inventory"]:openOtherInventory('pd_armory:1')
end)

RegisterNetEvent('esx_policejob:impoundRadial')
AddEventHandler('esx_policejob:impoundRadial', function()
	local veh = ESX.Game.GetClosestVehicle()
	ImpoundVehicle(veh)
end)

RegisterNetEvent('esx_policejob:CheckInventoryRadial')
AddEventHandler('esx_policejob:CheckInventoryRadial', function()
	local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
	TriggerServerEvent('esx_policejob:getPlayerID', GetPlayerServerId(closestPlayer), closestDistance, 'search')
end)

RegisterNetEvent('esx_policejob:OpenInventoryRadial')
AddEventHandler('esx_policejob:OpenInventoryRadial', function(dist, id)
	local distance = dist
	local pid = id
	if distance ~= -1 and distance<1 then
		exports["mf-inventory"]:openOtherInventory(pid)
		exports['mythic_notify']:DoHudText('inform', 'Searching subject')
	else
		exports['mythic_notify']:DoHudText('error', 'No one to search!')
	end
end)

RegisterNetEvent('esx_policejob:FriskRadial')
AddEventHandler('esx_policejob:FriskRadial', function()
	local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
	TriggerServerEvent('esx_policejob:getPlayerID', GetPlayerServerId(closestPlayer), closestDistance, 'frisk')
end)

RegisterNetEvent('esx_policejob:Frisk')
AddEventHandler('esx_policejob:Frisk', function(dist, id)
	local distance = dist
	local pid = id
	local clean = true 
	if distance ~= -1 and distance<1 then
		exports["mf-inventory"]:getInventoryItems(pid,function(items)
			for i, value in pairs(items) do
				if string.match(value.name, 'WEAPON') and value.name ~= 'WEAPON_FLASHLIGHT' then
					clean = false
					exports['mythic_notify']:DoHudText('inform', 'You feel a bulge')
					break
				end
			end
		end)
	...
	Citizen.Wait(1000)
	if clean then
		exports['mythic_notify']:DoHudText('inform', 'You found nothing')
	end
end)

RegisterNetEvent('esx_policejob:setLivery')
AddEventHandler('esx_policejob:setLivery', function(index)
	print('triggered')
	local veh = GetVehiclePedIsIn(GetPlayerPed(-1))
  	local livery = index
	if playerInService and atBennys then
		SetVehicleLivery(veh, tonumber(livery))
	end
end)

RegisterNetEvent('esx_policejob:runPlateRadial')
AddEventHandler('esx_policejob:runPlateRadial', function()
	print('triggered')
	local player = GetPlayerPed(-1)
	local veh,dis = ESX.Game.GetClosestVehicle(GetEntityCoords(player))
	local plate = GetVehicleNumberPlateText(veh)
	if veh ~= nil and dis <3 then	
		ESX.TriggerServerCallback('onyx:isStolen', function(isStolen)
			if isStolen then
				exports['okokNotify']:Alert("Dispatch:",''..plate..' is reported stolen', 15000, 'warning')
			else
				exports['okokNotify']:Alert("Dispatch:",''..plate..' comes back clean', 15000, 'success')
			end
		end, plate)
	end
end)

RegisterNetEvent('esx_policejob:bennysRepairRadial')
AddEventHandler('esx_policejob:bennysRepairRadial', function()
	print('passed arg')
	bennysRepair()
end)

function bennysRepair()
	local veh = GetVehiclePedIsIn(GetPlayerPed(-1), false)
	if playerInService and veh ~= -1 then 
		FreezeEntityPosition(veh, true)
		SetVehicleFixed(veh)
		exports['progressBars']:startUI(5000, 'Repairing Vehicle')
		Citizen.Wait(5000)
...
RegisterNetEvent('esx_policejob:handCuffRadial')
AddEventHandler('esx_policejob:handCuffRadial', function()
	local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
	if closestDistance ~= -1 and closestDistance<1 then
		TriggerServerEvent('esx_policejob:handcuff', GetPlayerServerId(closestPlayer))
	end
end)
...
RegisterNetEvent('esx_policejob:dragRadial')
AddEventHandler('esx_policejob:dragRadial', function()
	local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
	TriggerServerEvent('esx_policejob:drag', GetPlayerServerId(closestPlayer))
end)
...
--added
RegisterNetEvent('esx_policejob:putInVehicleRadial')
AddEventHandler('esx_policejob:putInVehicleRadial', function()
	local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
	TriggerServerEvent('esx_policejob:putInVehicle', GetPlayerServerId(closestPlayer))
end)
...
RegisterNetEvent('esx_policejob:OutVehicleRadial')
AddEventHandler('esx_policejob:OutVehicleRadial', function()
	local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
	TriggerServerEvent('esx_policejob:OutVehicle', GetPlayerServerId(closestPlayer))
end)
...
function ImpoundVehicle(vehicle)
	if playerInService then
		...
		exports['progressBars']:startUI(3000, "Calling for impound")
		Citizen.Wait(3000)
		ESX.Game.DeleteVehicle(vehicle)
		exports['mythic_notify']:DoHudText('success', 'Successfully Impounded')
		currentTask.busy = false
	end
	...
	--TriggerServerEvent('esx_policejob:addImpound', vehname, plate, location)
end
