...
function setSignal(weather) {
    let weatherIcon = "fas fa-signal"
    switch (weather) {
        case "EXTRASUNNY":
        case "CLEAR":
            weatherIcon = "fas fa-signal"
            break;
        case "THUNDER":
            weatherIcon = "fas fa-signal"
            break;
    ...
    $('.1status-bar-weather').empty();
    $('.1status-bar-weather').append(`<i class="${weatherIcon}"></i>`);
}
...
function addVehicles(vehicleData, showCarPayments) {
    ...
    for (let vehicle of Object.keys(vehicleData)) {
                    ...
                    <div class="row">
                        <div class="col s12"> 
                            <ul class="collection">
                                ...
                                <li class="collection-item"><i class="fas fa-oil-can"></i> ${vehicleData[vehicle].enginePercent}% Motor</li>
                                <li class="collection-item"><i class="fas fa-car-crash"></i> ${vehicleData[vehicle].bodyPercent}% Kasa</li>
                                `
        ...
        vehicleElement += `<button class="waves-effect waves-light btn-small garage-track" data-plate="${vehicleData[vehicle].plate}"><i class="fas fa-map-marker-alt"></i> Locate</button>
                        </div>
                    </div>
                </div>
            </li>
...
function openContainer(containerName, fadeInTime = 500, ...args) {
    ...
    $("." + containerName + "-container").hide().slideDown((currentContainer !== containerName ? fadeInTime : 0));
...
function closeContainer(containerName, fadeOutTime = 500) {
    $.when($("." + containerName + "-container").slideDown(fadeOutTime).hide()).then(function () {
        if (containerName === "home")
...
function openPhoneShell() {
    $(".phone-shell").add($('.phone-screen')).slideDown();
}

function closePhoneShell() {
    $(".phone-shell").add($('.phone-screen')).slideUp();
    $(".phone-screen .hidden-buttons").each(function () {
        $(this).hide().slideUp(150);
    });
}
