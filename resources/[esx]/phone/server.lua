...
AddEventHandler('phone:addContact', function(name, number)
    ...
    MySQL.Async.execute('INSERT INTO phone_contacts (identifier, name, number) VALUES (@identifier, @name, @number)', {
        ...
        MySQL.Async.fetchAll('SELECT * FROM phone_contacts WHERE identifier = @identifier', { ['@identifier'] = identifier }, function(contacts)
...
AddEventHandler('phone:sendSMSNPC', function(message)
    ...
    local target = getNumberPhone(xPlayer.getIdentifier())
    ...
    TriggerClientEvent('phone:newSMS', src, 1, mynumber)
    --TriggerClientEvent('refreshSMS', receiverID)  
    --else
    -- Mythic notify the source that they cannot text themselves
    --end
...
RegisterCommand("pagerr", function()
    TriggerClientEvent("phone:triggerPager")
end)
...
AddEventHandler('getAccountInfo', function()
    ...
    local inbank = player.getAccount('bank').money
    local licenceTable = {}
...
RegisterCommand("telefonara", function(source, args, raw)
    local src = source
    local pnumber = args[1]
    local xPlayer = ESX.GetPlayerFromId(source)
