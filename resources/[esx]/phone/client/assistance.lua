...
AddEventHandler('phoneGui', function()
  TriggerEvent('jobssystem:current', function(job)
    if ESX.PlayerData.job.name == nil or ESX.PlayerData.job.name == "unemployed" or ESX.PlayerData.job.name == "trucker" then
...
AddEventHandler('phone:assistNotify', function(id, service, player)
  TriggerEvent('jobssystem:current', function(job)
    ...
    elseif (ESX.PlayerData.job.name == 'police' or job == 'news') and (service == 'AIcall27' or service == 'AIcall26' or service == 'AIcall25' or service == 'AIcall24' or service == 'AIcall23' or service == 'AIcall' or service == 'AIcall2' or service == 'AIcall3a' or service == 'AIcall3' or service == 'AIcall4' or service == 'AIcall5' or service == 'AIcall6' or service == 'AIcall7' or service == 'AIcall8' or service == 'AIcall17' or service == 'AIcall18' or service == 'AIcall19' or service == 'AIcall20' or service == 'AIcall21' or service == 'AIcall22' or service == 'AIcall9') then
      if job == 'news' then
        addBlip({service = service, player = player, id = id,jobType = "news"})
...
Citizen.CreateThread(function()
  ...
  while true do
    ...
    if nearEntertainment() and ESX.PlayerData.job.name == "entertainer" then
    ...
    elseif ESX.PlayerData.job.name == "news" then
      local dist = 0
      if lastBlip.x then 
      ...
      if ESX.PlayerData.job.name == "entertainer" then
        Citizen.Wait(300000)
      end
    end
      ...
      if (key ~= nil and item ~= nil) then
        ...
        if #(pos - vector3( item.pos.x, item.pos.y, item.pos.z)) < 50.0001 then
          ...
          elseif item.jobType == "news" then 
            if ESX.PlayerData.job.name == "news" then
              TriggerServerEvent('phone:assistRemove', item.id, item.jobType) -- Send message of clear to others
              clearBlip(item)
              if GetTimeDifference(curTime, item.timestamp) > 2000 then
                if canPay then
