...
RegisterNUICallback('btnNotifyToggle', function(data, cb)
    ...
    if allowpopups then
      exports['mythic_notify']:DoHudText('success', 'Popups Enabled')
...
RegisterNUICallback('btnMute', function()
  if phoneNotifications then
    exports['mythic_notify']:DoHudText('error', 'Notifications Disabled.')
  else
    exports['mythic_notify']:DoHudText('success', 'Notifications Enabled.')
  end
...
RegisterNUICallback('btnGarage', function()
  ESX.TriggerServerCallback('esx_advancedgarage:getAllOwnedVehicles', function(ownedCars)
		if #ownedCars > 0 then
			TriggerEvent('phone:Garage', ownedCars)
		end
	end)
end)
...
AddEventHandler("phone:Garage", function(vehs)
  ...
  for ind, value in pairs(vehs) do
    ...
    if value.stored == true then
      currentGarage = value.location
    else
      currentGarage = 'Out'
    end
    coordlocation = nil
    allowspawnattempt = 0
    lastPayment = 0--financetimer
...
RegisterCommand('ornmail', function()
  TriggerEvent('phone:addnotification','Erotik Shop İsmail Abi', 'Elasynin dildo siparişi yola çıktı!')
end)
...
AddEventHandler("yellowPages:retrieveLawyersOnline", function(command)
    ...
    exports['mythic_notify']:DoHudText('inform', 'Searching for a Lawyer...', 2500, { ['background-color'] = '#0062ff' })
    Citizen.Wait(2000)
...
AddEventHandler('animation:phonecallstart', function()
  ...
  exports['mythic_notify']:DoHudText('inform', '[E] Toggles Call.')
  ...
  while callStatus ~= isNotInCall do
    ...
    if AnonCall then
       ...
       if dPB > 2.0 then
        exports['mythic_notify']:DoHudText('error', 'Moved too far.')
    ...
    if onhold then
      if count == 800 then
         count = 0
         exports['mythic_notify']:DoHudText('inform', 'Call On Hold.', 5000)
      end
    end
...
AddEventHandler('phone:makecall', function(pnumber)
  ...
  if callStatus == isNotInCall and not isDead and hasPhone() then
    ...
    exports['mythic_notify']:DoHudText('inform', 'It appears you are already in a call, injured or with out a phone, please type /hangup to reset your calls.', 10000)
...
AddEventHandler('phone:otherClientEndCall', function()
    ...
    exports['mythic_notify']:DoHudText('inform', 'Your call was ended!', 5000)
...
AddEventHandler('phone:answercall', function()
    ...
    answerCall()
    TriggerEvent("animation:phonecallstart")
    exports['mythic_notify']:DoHudText('inform', 'You have answered a call.')
    callTimer = 0
  else
    exports['mythic_notify']:DoHudText('error', 'You are not being called, injured, or you took too long.')
...
AddEventHandler('phone:initiateCall', function(srcID)
    exports['mythic_notify']:DoHudText('inform', 'You have started a call.', 5000)
...
AddEventHandler('phone:addToCall', function(voipchannel)
  ...
  exports['mumble-voip']:addPlayerToCall(tonumber(voipchannel))
end)
--accountinfo fix here todo
RegisterNUICallback('accountInformation', function()
  local xPlayer = nil
  ESX.TriggerServerCallback('esx:getCharInfo', function(player)
		xPlayer = player
	end)
  ...
  local responseObject = {
    cash = xPlayer.money,--exports["isPed"]:isPed("mycash"),
    bank = xPlayer.bank,--exports["isPed"]:isPed("mybank"),
    job = xPlayer.job,--exports["isPed"]:isPed("myjob"),
...
function initiatingCall()
  ...
  exports['mythic_notify']:DoHudText('inform', 'You are making a call, please hold.', 7500)
...
function receivingCall(callFrom)
  ...
  while (callTimer > 0 and callStatus == isReceivingCall) do
    if hasPhone() then
      exports['mythic_notify']:DoHudText('inform', 'Call from: ' .. callFrom .. " /ans or /h", 5000)
      if phoneNotifications then
...
AddEventHandler('phone:removefromToko', function(playerRadioChannel)
  exports['mumble-voip']:removePlayerFromCall(playerRadioChannel)
end)
...
AddEventHandler('phone:holdToggle', function()
  ...
  if myID ~= 0 then
    if not onhold then
      ...
      exports['mythic_notify']:DoHudText('infrom', 'No longer on hold.', 5000)
    ...
    if mySourceID ~= 0 then
      if not onhold then
        ...
        exports['mythic_notify']:DoHudText('inform', 'No longer on hold.')
        TriggerServerEvent("OnHold:Server",mySourceID,false)
        onhold = false
...
AddEventHandler('Client:UpdateTweet', function(data, handle2)
    ...
    if string.find(message,handle) then
        ...
        if handle2 ~= handle then
            ...
            if phoneNotifications then
                PlaySound(-1, "Event_Start_Text", "GTAO_FM_Events_Soundset", 0, 0, 1)
                exports['mythic_notify']:DoHudText('inform', 'You were just mentioned in a tweet on your phone.', 5000)
            end
        end
    end
...
RegisterCommand('denemekomt', function()
  TriggerEvent('phone:triggerPager')
end)
