...
function checkDistanceFromStore()
	local playerCoords = GetEntityCoords(PlayerPedId())
	distanceFromStore = GetDistanceBetweenCoords(playerCoords.x, playerCoords.y, playerCoords.z, Config.Zones.LicenseShops.Coords[1].x, Config.Zones.LicenseShops.Coords[1].y, Config.Zones.LicenseShops.Coords[1].z, true)
	if distanceFromStore < 5 then
		DrawText3Ds(-550.8488, -191.0132, 38.69617, "Press 'E' to enter License Shop")
...
Citizen.CreateThread(function()
	while true do
		...
		if checkDistanceFromStore() < 2 then 
			isInMarker = true
		else
			isInMarker = false
		end
