...
MySQL.ready(function()
	if Config.Main.ParkVehicles then
		...
		MySQL.Async.execute('UPDATE shared_vehicles SET `stored` = true WHERE `stored` = @stored', {
			...
			if rowsChanged > 0 then
				print(('esx_advancedgarage: %s shared vehicle(s) have been stored!'):format(rowsChanged))
			end
		end)
		MySQL.Async.execute('UPDATE owned_vehicles SET `location` = @newLocation WHERE `location` = @location AND `job` = @job', {
			...
			if rowsChanged > 0 then
				print(('esx_advancedgarage: %s vehicle(s) have been moved!'):format(rowsChanged))
			end
		end)
		
		MySQL.Async.execute('UPDATE shared_vehicles SET `location` = @newLocation WHERE `location` = @location', {
			['@location'] = '',
			['@newLocation'] = 'policeGarage'
		}, function(rowsChanged)
			if rowsChanged > 0 then
				print(('esx_advancedgarage: %s shared vehicle(s) have been moved!'):format(rowsChanged))
			end
		end)
		MySQL.Async.execute('UPDATE owned_vehicles SET `location` = @newLocation WHERE `location` = @location AND `job` = @job', {
			['@location'] = '',
...
ESX.RegisterServerCallback('esx_advancedgarage:isVehicleOwned', function(source, cb, plate, location)
...
ESX.RegisterServerCallback('esx_advancedgarage:getAllOwnedVehicles', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
	local ownedCars = {}
...
ESX.RegisterServerCallback('esx_advancedgarage:getSharedVehicles', function(source, cb, job, type)
...
ESX.RegisterServerCallback('esx_advancedgarage:getOwnedVehicles', function(source, cb, job, type, location)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	elseif job == 'police' then
		if type == 'cars' then
			...
			MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner = @owner AND Type = @Type AND job = @job AND category = @category AND location = @location', {
				['@owner'] = xPlayer.identifier,
				['@Type'] = 'car',
				['@job'] = 'police',
				['@category'] = 'cars',
	...
	elseif job == 'towing' then
		if type == 'trucks' then
			...
			MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner = @owner AND Type = @Type AND job = @job', {
				...
				for _,v in pairs(data) do
					local vehicle = json.decode(v.vehicle)
					table.insert(ownedTowCars, {vehicle = vehicle, plate = v.plate, vehName = v.name, fuel = v.fuel, stored = v.stored})
				end
				cb(ownedTowCars)
			end)
		end
...
AddEventHandler('esx_advancedgarage:setVehicleState', function(plate, state, location)
	local xPlayer = ESX.GetPlayerFromId(source)
