...
function SetPropertyOwned(name, price, rented, owner)
	MySQL.Async.execute('INSERT INTO owned_properties (name, price, rented, owner) VALUES (@name, @price, @rented, @owner)', {
		...
		local uniqueIdentifier = name..":"..xPlayer.identifier  -- Unique identifier for this inventory.
...
RegisterNetEvent('esx_property:GiveApartment')
AddEventHandler('esx_property:GiveApartment', function(propertyName)
	local xPlayer  = ESX.GetPlayerFromId(source)
	SetPropertyOwned(propertyName, 0, false, xPlayer.identifier)
end)

RegisterNetEvent('esx_property:openInventoryProperty')
AddEventHandler('esx_property:openInventoryProperty', function(propertyName)
	local xPlayer  = ESX.GetPlayerFromId(source)
	TriggerClientEvent('esx_property:openInventoryProperty', source, xPlayer.identifier)
end)
