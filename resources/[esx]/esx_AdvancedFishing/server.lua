...
ESX.RegisterUsableItem('turtlebait', function(source)
	...
	if xPlayer.getInventoryItem('fishingrod').count > 0 then
		...
		TriggerClientEvent('fishing:message', _source, "You attach the turtle bait onto your fishing rod")
	else
...
ESX.RegisterUsableItem('turtle', function(source)
	...
	if xPlayer.getInventoryItem('fishingrod').count > 0 then
		...
		TriggerClientEvent('fishing:message', _source, "You attach the turtle meat onto the fishing rod")
	else
		TriggerClientEvent('fishing:message', _source, "You dont have a fishing rod")
	end
...
AddEventHandler('fishing:catch', function(bait, onPier)
	
	_source = source
	local weight = 2
	local rnd = math.random(1,100)
	xPlayer = ESX.GetPlayerFromId(_source)
	if bait == "turtle" and not onPier then
		if rnd >= 78 then
			if rnd >= 94 then
				TriggerClientEvent('fishing:setbait', _source, "none")
				TriggerClientEvent('fishing:message', _source, "It was huge and it broke your fishing rod!")
				...
				if xPlayer.getInventoryItem('turtle').count > 4 then
					TriggerClientEvent('fishing:message', _source, "You cant hold more turtles")
				else
					TriggerClientEvent('fishing:message', _source, "You caught a turtle, don't let the cops see...")
			...
			if rnd >= 75 then
				if xPlayer.getInventoryItem('fish').count > 100 then
					TriggerClientEvent('fishing:message', _source, "You cant hold more fish")
				else
					weight = math.random(4,9)
					TriggerClientEvent('fishing:message', _source, "You caught a fish: " .. weight .. "kg")
		...
		if bait == "shark" and not onPier then
			if rnd >= 82 then
				if rnd >= 91 then
					TriggerClientEvent('fishing:setbait', _source, "none")
					TriggerClientEvent('fishing:message', _source, "It was huge and it broke your fishing rod!")
					...
					if xPlayer.getInventoryItem('shark').count > 0  then
						TriggerClientEvent('fishing:setbait', _source, "none")
						TriggerClientEvent('fishing:message', _source, "You cant hold more sharks")
					else
						TriggerClientEvent('fishing:message', _source, "You caught a shark, dont let the cops see...")
