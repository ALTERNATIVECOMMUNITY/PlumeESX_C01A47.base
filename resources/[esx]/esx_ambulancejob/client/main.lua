...
Citizen.CreateThread(function()
	while true do 
		--add if disabling loop here
		local playerPed = PlayerPedId()
        if isDead or GetEntityHealth(playerPed) <= 0 then
			if not deathSet then
				OnPlayerDeath()
			end
			SetEntityInvincible(playerPed, true)
            SetPedDiesInWater(PlayerPedId(), false)
			--add food and water to player here
			myanim = "dead_a"
            Citizen.Wait(100)
            ...
            if veh then
                ...
                for index = 1, GetVehicleModelNumberOfSeats(vehmodel) do
                    if GetPedInVehicleSeat(veh,index) == playerPed then
                        seat = index
                    end
                end
            end
            ...
            Wait(100)
            if veh then
                TaskWarpPedIntoVehicle(playerPed,veh,index)
            end
			...
			while isDead do
                if GetEntityHealth(playerPed) < 105.0  then
                    SetEntityHealth(playerPed, 105)
                end
                ...
                elseif not InVeh() then
                    if (GetEntitySpeed(PlayerPedId()) > 0.3 and not inwater and not IsEntityPlayingAnim(PlayerPedId(), "savecouch@", 't_sleep_loop_couch', 1)) or (not IsEntityPlayingAnim(PlayerPedId(), "dead", myanim, 1) and not inwater and not IsEntityPlayingAnim(PlayerPedId(), "savecouch@", 't_sleep_loop_couch', 1)) then
						print('deathAnimB')
                        TriggerEvent("deathAnim")
...
RegisterNetEvent('esx_ambulancejob:givePedArmor')
AddEventHandler('esx_ambulancejob:givePedArmor', function(amount)
	local player = PlayerPedId()
	local currentArmor = GetPedArmour(player)
	exports['progressBars']:startUI(7000, "Putting on vest")
	Citizen.Wait(7000)
	SetPedArmour(player, currentArmor+amount)
end)
...
AddEventHandler('deathAnim', function()
    if not tryingAnim and not enteringveh and not InVeh() and isDead then
        tryingAnim = true
