...
AddEventHandler('esx_ambulancejob:revive', function(playerId)
	...
	if xPlayer and xPlayer.job.name == 'mount_zonah' then
		...
		if xTarget then
			if deadPlayers[playerId] then
				if Config.ReviveReward > 0 then
					...
					xTarget.triggerEvent('mythic_hospital:client:RemoveBleed')
    				xTarget.triggerEvent('mythic_hospital:client:ResetLimbs')
				else
...
AddEventHandler('esx_ambulancejob:heal', function(target, type)
	...
	if xPlayer.job.name == 'mount_zonah' then
