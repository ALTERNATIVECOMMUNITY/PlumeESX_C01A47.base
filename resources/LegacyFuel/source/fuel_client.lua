...
RegisterKeyMapping("fuel:refuelFromPump", "Fuel", "keyboard", "e")

RegisterNetEvent("Eye:pump")
AddEventHandler("Eye:pump", function(msg)
	local pos = GetEntityCoords(GetPlayerPed(-1))
	local veh,dist = ESX.Game.GetClosestVehicle(pos)
	if (isNearPump and not bMenuOpen and veh ~= 0 and dist <3) then
		local ped = PlayerPedId()
